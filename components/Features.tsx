...
const FeatureBlock: React.FC<{
  ...
  return (
    <div className={`flex flex-col gap-12 py-20 lg:flex-row lg:items-center lg:gap-20 ${reversed ? 'lg:flex-row-reverse' : ''}`}>
      {/* Visual Side */}
      ...
      <div className="flex-1 space-y-6">
        ...
        <button className="rounded-full bg-indigo-500/10 px-6 py-2.5 text-sm font-medium text-indigo-300 hover:bg-indigo-500/20 transition-colors border border-indigo-500/20">
          {ctaText}
        </button>
      </div>
    </div>
  ...
  return (
    <section id="features" className="mx-auto max-w-6xl px-4 md:px-6">
      <FeatureBlock 
        ...
        ctaText="Sing up free" // Typo intentional to match image or corrected "Sign up free"
      />
