...
export const Placeholder: React.FC<PlaceholderProps> = ({ label, className = "", height = "h-64" }) => {
  return (
    <div className={`relative w-full ${height} overflow-hidden rounded-2xl border border-dashed border-white/10 bg-white/5 flex flex-col items-center justify-center group ${className}`}>
      ...
      <div className="z-10 flex flex-col items-center text-gray-500 group-hover:text-violet-400 transition-colors duration-300">
        ...
        <span className="font-mono text-xs uppercase tracking-widest opacity-70 mb-2">{label}</span>
